...
/**
 * Custom odometry class for swerve drive robots that calculates the robot's
 * position based on individual module displacements, treating each module's
 * movement as an arc instead of a straight line.
 * 
 * Inspired by FRC Team 581's implementation of FRC Team 1690's custom odometry
 */
public class CustomOdometry extends SwerveDriveOdometry {
    ...
    public CustomOdometry(SwerveDriveKinematics kinematics, Rotation2d gyroAngle,
    ...
    /**
     * Calculates the displacement of a swerve module based on its previous and
     * current positions as an arc movement.
     * 
    ...
    private static Translation2d getModuleDisplacement(SwerveModulePosition previousWheelPosition,
        ...
        // distances
        double differenceVectorAngle = currentWheelPosition.angle.getRadians()
                - previousWheelPosition.angle.getRadians();
        double differenceVectorLength = currentWheelPosition.distanceMeters - previousWheelPosition.distanceMeters;
    ...
    /**
     * Updates the robot's pose based on the current gyro angle and module
     * positions.
     * 
    ...
    public Pose2d update(Rotation2d gyroAngle, SwerveModulePosition[] modulePositions) {
        ...
        for (int i = 0; i < numberOfModules; i++) {
            fieldRelativeModulePosesOfPreviousPose[i] = robotPose.transformBy(
        ...
        for (int i = 0; i < numberOfModules; i++) {
            fieldRelativeModuleDisplacements[i] = fieldRelativeModulePosesOfPreviousPose[i]
                    .transformBy(new Transform2d(moduleDisplacements[i], Rotation2d.kZero)).getTranslation();
        }
        ...
        for (int i = 0; i < numberOfModules; i++) {
            sumOfFieldRelativeModuleDisplacements = sumOfFieldRelativeModuleDisplacements
                    .plus(fieldRelativeModuleDisplacements[i]);
        }
        ...
        for (int i = 0; i < numberOfModules; i++) {
            Logger.recordOutput("CustomOdometry/FieldRelativeModuleDisplacements/" + i,
                    new Pose2d(fieldRelativeModuleDisplacements[i], gyroAngle.plus(modulePositions[i].angle)));
        }
